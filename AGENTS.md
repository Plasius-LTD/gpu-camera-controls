...
### Local Notes
- Keep camera input and gesture handling framework-agnostic and independent from
  Three.js.
- Do not import or copy `camera-controls`; use it only as an interaction
  reference.
- Keep camera math, matrix generation, projection handling, and rig primitives
  delegated to `@plasius/gpu-camera`.
- Add ADR updates when touch, pointer, analog, or damping behavior changes.
